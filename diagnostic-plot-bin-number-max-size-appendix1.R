...
#plot of the growth distributions#####
sizes<-seq(100,900,150)
...
for(i in 1:length(bins)){
  ...
  my_ipm<-define_kernel(
    proto_ipm = my_ipm,
    name          = "P",#define what IPM we're putting this kernel in
    family        = "CC",#the variable in this kernel starts and ends Continuous
    formula       = g, #the formula for the kernel
    #define formulas
  ...
  my_ipm<-define_domains(
    proto_ipm = my_ipm,#define the upper and lower ranges, and how many midpoints
    length = c(0,
               1100,
               bins[i]))
  
  my_ipm<-define_pop_state(proto_ipm = my_ipm,
                           n_length = rep(1/bins[i],bins[i]))
  
  my_ipm<-make_ipm(proto_ipm=my_ipm,
  ...
  lam <- lambda(my_ipm)
  lambdas.bins<-c(lambdas.bins,lam)
}

#plot of lambda and bins#####
setwd("~/Documents/Thesis/good code")
png("bin number graphs.png", width= 2404, height= 1600, units="px", res = 300)
plot(bins,lambdas.bins,xlab="Number of mesh points",
...
for(i in 1:length(size)){
  ...
  lam <- lambda(my_ipm)
  lambdas.size<-c(lambdas.size,lam)
}

#plot of lambda and size####
setwd("~/Documents/Thesis/good code")
png("upper limit  graphs.png", width= 2404, height= 1600, units="px", res = 300)
plot(size,lambdas.size,xlab="Upper Limit (mm)",
     ylab="Population growth rate")
abline(h=1,lty=2)
dev.off()
...
setwd("~/Documents/Thesis/good code")
png("max age diagnostic.png", width= 2404, height= 1600, units="px", res = 300)
plot(ages,lambda.ages,xlab="Maximum Age (years)",
     ylab="Population growth rate")
dev.off()
