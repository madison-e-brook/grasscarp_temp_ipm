...
#vary survival at different degree days#####
#3560 dd####
survs<-seq(0.5,0.9,0.05)
...
for(i in 1:length(survs)){
  ...
  lambdas.3560<-c(lambdas.3560,lambda(test_ipm))
}

plot(survs,lambdas.3560)

l3560<-data.frame(surv.max=survs,lambdas=lambdas.3560)
l3560
setwd("~/Documents/Thesis/good code")
write.csv(l3560,"surv range at 3560dd.csv",row.names = F)
...
for(i in 1:length(survs)){
  ...
  parameters(use_proto)<-constant_list_new
  test_ipm<-make_ipm(proto_ipm=use_proto, iterations = 100)
  lambdas.2330<-c(lambdas.2330,lambda(test_ipm))
}

plot(survs,lambdas.2330)

l2330<-data.frame(surv.max=survs,lambdas=lambdas.2330)
l2330
setwd("~/Documents/Thesis/good code")
write.csv(l2330,"surv range at 2330dd.csv",row.names = F)
...
for(i in 1:length(survs)){
  ...
  parameters(use_proto)<-constant_list_new
  test_ipm<-make_ipm(proto_ipm=use_proto, iterations = 100)
  lambdas.4280<-c(lambdas.4280,lambda(test_ipm))
}

plot(survs,lambdas.4280)

l4280<-data.frame(surv.max=survs,lambdas=lambdas.4280)
l4280
setwd("~/Documents/Thesis/good code")
write.csv(l4280,"surv range at 4280dd.csv",row.names = F)
